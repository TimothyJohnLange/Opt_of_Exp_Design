...
from matplotlib.pyplot import plot, xlim, ylim, legend, figure, show
#get_ipython().magic(u'pylab inline')

# ####Define Constants

p = [45*8, 45*8 - 10, 80, 100] #Time limit for each student
...
for data_type in data_types:
    ...
    for n, experiment in enumerate(experiments):
        ...
        if val != None:
            new_data_type = 0
            if val <= 25:
                all_adjusted_val = range(start, int(val)*2 + 1, interval)
            else:
                all_adjusted_val = range(start, int(val)*2 + 1, 10)
...
for student in range(4):
    ...
    index = all_p_adjust_per_stu[student].index(p_reset[student])
